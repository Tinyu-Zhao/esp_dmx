...
void app_main() {
    ...
    QueueHandle_t queue;
    ESP_ERROR_CHECK(dmx_driver_install(dmx_num, DMX_MAX_PACKET_SIZE, 1, &queue, 1));
            ...
            if (packet.status == DMX_OK) {
                ...
                if (timer >= 1000000) {
                    ESP_LOGI(TAG,
                             ...
                             "%ius Packet: %.2fms",
                             packet.start_code, packet.size, data[1], packet.timing.brk, packet.timing.mab,
                             packet.duration / 1000.0);
                    timer -= 1000000;
                }
