...
static size_t rdm_rhd_get_supported_parameters(dmx_port_t dmx_num, const rdm_parameter_definition_t *definition,
    ...
    return rdm_write_ack(dmx_num, header, definition->get.response.format, pids, pdl);
}

static size_t rdm_rhd_get_parameter_description(dmx_port_t dmx_num, const rdm_parameter_definition_t *definition,
                                                const rdm_header_t *header) {
    ...
    uint16_t pid;
    if (!rdm_read_pd(dmx_num, definition->get.request.format, &pid, sizeof(pid))) {
    ...
    const rdm_parameter_definition_t *requested_definition = rdm_definition_get(dmx_num, header->sub_device, pid);
    ...
    strncpy(pd.description, requested_definition->description, RDM_ASCII_SIZE_MAX);

    return rdm_write_ack(dmx_num, header, definition->get.response.format, &pd, sizeof(pd));
}

bool rdm_register_supported_parameters(dmx_port_t dmx_num, rdm_callback_t cb, void *context) {
    ...
    static const rdm_parameter_definition_t definition = {
        ...
        .get         = {.handler = rdm_rhd_get_supported_parameters, .request.format = NULL, .response.format = "w"},
...
bool rdm_register_parameter_description(dmx_port_t dmx_num, rdm_callback_t cb, void *context) {
    DMX_CHECK(dmx_num < DMX_NUM_MAX, false, "dmx_num error");
    DMX_CHECK(dmx_driver_is_installed(dmx_num), false, "driver is not installed");
...
size_t rdm_get_parameter_description(dmx_port_t dmx_num, rdm_pid_t pid, rdm_parameter_description_t *parameter) {
    DMX_CHECK(dmx_num < DMX_NUM_MAX, false, "dmx_num error");
    DMX_CHECK(!(pid < RDM_PID_MANUFACTURER_SPECIFIC_BEGIN || pid > RDM_PID_MANUFACTURER_SPECIFIC_END), false,
              "pid error");
    DMX_CHECK(parameter != NULL, false, "parameter is null");
    DMX_CHECK(dmx_driver_is_installed(dmx_num), false, "driver is not installed");

    // Ensure the request PID is known
    const rdm_parameter_definition_t *requested_definition = rdm_definition_get(dmx_num, RDM_SUB_DEVICE_ROOT, pid);
    if (requested_definition == NULL) {
        return 0;
    }
    ...
    strncpy(parameter->description, requested_definition->description, RDM_ASCII_SIZE_MAX);
