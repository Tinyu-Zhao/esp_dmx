...
static size_t rdm_rhd_set_reset_device(dmx_port_t dmx_num, const rdm_parameter_definition_t *definition,
                                       const rdm_header_t *header) {
    // Verify the reset state is valid
    uint8_t reset;
    if (!rdm_read_pd(dmx_num, definition->set.request.format, &reset, sizeof(reset))) {
    ...
    // Set the mute type
    dmx_parameter_set(dmx_num, RDM_SUB_DEVICE_ROOT, header->pid, &reset, sizeof(reset));

    // This parameter clears the mute flag
    const uint8_t mute = 0;
    dmx_parameter_set(dmx_num, RDM_SUB_DEVICE_ROOT, RDM_PID_DISC_MUTE, &mute, sizeof(mute));

    return rdm_write_ack(dmx_num, header, NULL, NULL, 0);
}

bool rdm_register_identify_device(dmx_port_t dmx_num, rdm_callback_t cb, void *context) {
    ...
    static const rdm_parameter_definition_t definition = {
        ...
        .get         = {.handler = rdm_simple_response_handler, .request.format = NULL, .response.format = "b$"},
        .set         = {.handler = rdm_simple_response_handler, .request.format = "b$", .response.format = NULL},
...
size_t rdm_get_identify_device(dmx_port_t dmx_num, bool *identify) {
    ...
    return dmx_parameter_copy(dmx_num, RDM_SUB_DEVICE_ROOT, RDM_PID_IDENTIFY_DEVICE, identify, sizeof(uint8_t));
}
...
bool rdm_set_identify_device(dmx_port_t dmx_num, const bool identify) {
    ...
    const rdm_pid_t pid = RDM_PID_IDENTIFY_DEVICE;
    if (!dmx_parameter_set(dmx_num, RDM_SUB_DEVICE_ROOT, pid, &identify, sizeof(uint8_t))) {
        return false;
    }
...
bool rdm_register_reset_device(dmx_port_t dmx_num, rdm_callback_t cb, void *context) {
    DMX_CHECK(dmx_num < DMX_NUM_MAX, false, "dmx_num error");
    DMX_CHECK(cb != NULL, false, "cb is null");
    DMX_CHECK(dmx_driver_is_installed(dmx_num), false, "driver is not installed");
    ...
    static const rdm_parameter_definition_t definition = {
        ...
        .set         = {.handler = rdm_rhd_set_reset_device, .request.format = "b$", .response.format = NULL},
        .pdl_size    = sizeof(uint8_t),
        .max_value   = 0,
        .min_value   = 0,
        .units       = RDM_UNITS_NONE,
...
size_t rdm_get_reset_device(dmx_port_t dmx_num, uint8_t *reset) {
    ...
    return dmx_parameter_copy(dmx_num, RDM_SUB_DEVICE_ROOT, RDM_PID_RESET_DEVICE, reset, sizeof(*reset));
}
