...
static size_t rdm_rhd_discovery(dmx_port_t dmx_num, const rdm_parameter_definition_t *definition,
    ...
    if (header->pid == RDM_PID_DISC_UNIQUE_BRANCH) {
        ...
        dmx_parameter_copy(dmx_num, RDM_SUB_DEVICE_ROOT, RDM_PID_DISC_MUTE, &is_muted, sizeof(is_muted));
        if (is_muted) {
            return 0;
        }
        ...
        if (!rdm_read_pd(dmx_num, definition->get.request.format, &branch, sizeof(branch))) {
        ...
        if (rdm_uid_is_lt(this_uid, &branch.lower_bound) || rdm_uid_is_gt(this_uid, &branch.upper_bound)) {
        ...
        dmx_parameter_set(dmx_num, RDM_SUB_DEVICE_ROOT, header->pid, &set_mute, sizeof(set_mute));
        ...
        return rdm_write_ack(dmx_num, header, definition->get.response.format, &mute, sizeof(mute));
    }
}
...
bool rdm_register_disc_unique_branch(dmx_port_t dmx_num, rdm_callback_t cb, void *context) {
    ...
    static const rdm_parameter_definition_t definition = {
        ...
        .get         = {.handler = rdm_rhd_discovery, .request.format = "uu$", .response.format = NULL},
...
bool rdm_register_disc_mute(dmx_port_t dmx_num, rdm_callback_t cb, void *context) {
    ...
    uint8_t *un_mute = dmx_parameter_get_data(dmx_num, RDM_SUB_DEVICE_ROOT, RDM_PID_DISC_UN_MUTE);
    if (un_mute == NULL) {
        uint8_t init_value = 0;
        if (!dmx_parameter_add(dmx_num, RDM_SUB_DEVICE_ROOT, pid, DMX_PARAMETER_TYPE_DYNAMIC, &init_value,
                               sizeof(init_value))) {
            return false;
        }
    } else {
        if (!dmx_parameter_add(dmx_num, RDM_SUB_DEVICE_ROOT, pid, DMX_PARAMETER_TYPE_STATIC, un_mute,
                               sizeof(*un_mute))) {
            return false;
        }
    }
    ...
    static const rdm_parameter_definition_t definition = {
        .pid_cc      = RDM_CC_DISC,
        .ds          = RDM_DS_NOT_DEFINED,
        .get         = {.handler = rdm_rhd_discovery, .request.format = NULL, .response.format = "wv"},
        .set         = {.handler = NULL, .request.format = NULL, .response.format = NULL},
        .pdl_size    = 0,
        .max_value   = 0,
        .min_value   = 0,
...
bool rdm_register_disc_un_mute(dmx_port_t dmx_num, rdm_callback_t cb, void *context) {
    ...
    // Add the parameter as a new variable or as an alias to its counterpart
    uint8_t *mute = dmx_parameter_get_data(dmx_num, RDM_SUB_DEVICE_ROOT, RDM_PID_DISC_MUTE);
    if (mute == NULL) {
        uint8_t init_value = 0;
        if (!dmx_parameter_add(dmx_num, RDM_SUB_DEVICE_ROOT, pid, DMX_PARAMETER_TYPE_DYNAMIC, &init_value,
                               sizeof(init_value))) {
        ...
        if (!dmx_parameter_add(dmx_num, RDM_SUB_DEVICE_ROOT, pid, DMX_PARAMETER_TYPE_STATIC, mute, sizeof(*mute))) {
            return false;
        }
    }
