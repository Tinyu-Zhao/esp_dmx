...
bool rdm_send_disc_mute(dmx_port_t dmx_num, const rdm_uid_t *dest_uid, rdm_disc_mute_t *mute, rdm_ack_t *ack) {
    ...
    const rdm_request_t request = {
        .dest_uid = dest_uid, .sub_device = RDM_SUB_DEVICE_ROOT, .cc = RDM_CC_DISC_COMMAND, .pid = RDM_PID_DISC_MUTE};
        ...
        if (ack.type == RDM_RESPONSE_TYPE_ACK) {
          ...
          devices_remaining = (ack.err && ack.err != DMX_ERR_TIMEOUT);
                ...
                if (devices_remaining) {
                    ...
                    uint64_t mid = ((((uint64_t)branch->lower_bound.man_id << 32) | branch->lower_bound.dev_id) +
                                    (((uint64_t)branch->upper_bound.man_id << 32) | branch->upper_bound.dev_id)) /
                                   2;
...
static void rdm_disc_cb(dmx_port_t dmx_num, rdm_uid_t uid, int num_found, const rdm_disc_mute_t *mute, void *context) {
