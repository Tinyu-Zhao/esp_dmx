...
size_t rdm_send_get_device_info(dmx_port_t dmx_num, const rdm_uid_t *dest_uid, rdm_sub_device_t sub_device,
    ...
    const rdm_request_t request = {
        .dest_uid = dest_uid, .sub_device = sub_device, .cc = RDM_CC_GET_COMMAND, .pid = RDM_PID_DEVICE_INFO};

    const char *format = "x01x00wwdwbbwwb$";
    return rdm_send_request(dmx_num, &request, format, device_info, sizeof(*device_info), ack);
}
...
size_t rdm_send_get_software_version_label(dmx_port_t dmx_num, const rdm_uid_t *dest_uid, rdm_sub_device_t sub_device,
    ...
    const char *format = "a$";
    return rdm_send_request(dmx_num, &request, format, software_version_label, size, ack);
}
