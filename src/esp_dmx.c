...
esp_err_t dmx_driver_install(dmx_port_t dmx_num, uint16_t buffer_size, uint32_t queue_size, QueueHandle_t *dmx_queue,
    ...
    ESP_RETURN_ON_FALSE(dmx_num >= 0 && dmx_num < DMX_NUM_MAX, ESP_ERR_INVALID_ARG, TAG, "dmx_num error");
    ESP_RETURN_ON_FALSE(buffer_size > 0 && buffer_size <= DMX_MAX_PACKET_SIZE, ESP_ERR_INVALID_ARG, TAG,
                        "buffer_size error");
    ...
    if (p_dmx_obj[dmx_num] == NULL) {
        ...
        p_dmx_obj[dmx_num] = (dmx_obj_t *)heap_caps_calloc(1, sizeof(dmx_obj_t), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
        if (p_dmx_obj[dmx_num] == NULL) {
...
#endif
        p_dmx_obj[dmx_num]->buffer[0] = heap_caps_malloc(alloc_size, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    ...
    esp_err_t err = esp_intr_alloc(uart_periph_signal[dmx_num].irq, intr_alloc_flags, &dmx_intr_handler,
                                   p_dmx_obj[dmx_num], &p_dmx_obj[dmx_num]->intr_handle);
    if (err) {
        dmx_driver_delete(dmx_num);
        return err;
    }
...
esp_err_t dmx_set_mode(dmx_port_t dmx_num, dmx_mode_t dmx_mode) {
    ESP_RETURN_ON_FALSE(dmx_num >= 0 && dmx_num < DMX_NUM_MAX, ESP_ERR_INVALID_ARG, TAG, "dmx_num error");
    ESP_RETURN_ON_FALSE(dmx_mode >= 0 && dmx_mode < DMX_MODE_MAX, ESP_ERR_INVALID_ARG, TAG, "dmx_mode error");
    ESP_RETURN_ON_FALSE(p_dmx_obj[dmx_num] != NULL, ESP_ERR_INVALID_STATE, TAG, "driver not installed");
...
esp_err_t dmx_get_mode(dmx_port_t dmx_num, dmx_mode_t *dmx_mode) {
    ESP_RETURN_ON_FALSE(dmx_num >= 0 && dmx_num < DMX_NUM_MAX, ESP_ERR_INVALID_ARG, TAG, "dmx_num error");
    ESP_RETURN_ON_FALSE(dmx_mode != NULL, ESP_ERR_INVALID_ARG, TAG, "dmx_mode must not be null");
    ESP_RETURN_ON_FALSE(p_dmx_obj[dmx_num] != NULL, ESP_ERR_INVALID_STATE, TAG, "driver not installed");
...
#endif
    ESP_RETURN_ON_FALSE(dmx_num >= 0 && dmx_num < DMX_NUM_MAX, ESP_ERR_INVALID_ARG, TAG, "dmx_num error");
    ESP_RETURN_ON_FALSE(intr_io_num > -1 && GPIO_IS_VALID_GPIO(intr_io_num), ESP_ERR_INVALID_ARG, TAG,
                        "intr_io_num error");
    ESP_RETURN_ON_FALSE(p_dmx_obj[dmx_num] != NULL, ESP_ERR_INVALID_STATE, TAG, "driver not installed");
    ESP_RETURN_ON_FALSE(p_dmx_obj[dmx_num]->mode == DMX_MODE_READ, ESP_ERR_INVALID_STATE, TAG, "must be in read mode");
    ESP_RETURN_ON_FALSE(p_dmx_obj[dmx_num]->queue != NULL, ESP_ERR_INVALID_STATE, TAG, "queue is null");
    ESP_RETURN_ON_FALSE(p_dmx_obj[dmx_num]->intr_io_num == -1, ESP_ERR_INVALID_STATE, TAG, "sniffer already enabled");

    // add the isr handler
    esp_err_t err = gpio_isr_handler_add(intr_io_num, dmx_timing_intr_handler, p_dmx_obj[dmx_num]);
    if (err) return err;
...
esp_err_t dmx_sniffer_disable(dmx_port_t dmx_num) {
    ESP_RETURN_ON_FALSE(dmx_num >= 0 && dmx_num < DMX_NUM_MAX, ESP_ERR_INVALID_ARG, TAG, "dmx_num error");
    ESP_RETURN_ON_FALSE(p_dmx_obj[dmx_num] != NULL, ESP_ERR_INVALID_STATE, TAG, "driver not installed");
    ESP_RETURN_ON_FALSE(p_dmx_obj[dmx_num]->intr_io_num != -1, ESP_ERR_INVALID_STATE, TAG, "sniffer not enabled");
...
esp_err_t dmx_set_pin(dmx_port_t dmx_num, int tx_io_num, int rx_io_num, int rts_io_num) {
    ESP_RETURN_ON_FALSE(dmx_num >= 0 && dmx_num < DMX_NUM_MAX, ESP_ERR_INVALID_ARG, TAG, "dmx_num error");
    ESP_RETURN_ON_FALSE(tx_io_num < 0 || GPIO_IS_VALID_OUTPUT_GPIO(tx_io_num), ESP_ERR_INVALID_ARG, TAG,
                        "tx_io_num error");
    ESP_RETURN_ON_FALSE(rx_io_num < 0 || GPIO_IS_VALID_GPIO(rx_io_num), ESP_ERR_INVALID_ARG, TAG, "rx_io_num error");
    ESP_RETURN_ON_FALSE(rts_io_num < 0 || GPIO_IS_VALID_OUTPUT_GPIO(rts_io_num), ESP_ERR_INVALID_ARG, TAG,
                        "rts_io_num error");

    return uart_set_pin(dmx_num, tx_io_num, rx_io_num, rts_io_num, DMX_PIN_NO_CHANGE);
}

esp_err_t dmx_param_config(dmx_port_t dmx_num, const dmx_config_t *dmx_config) {
    ESP_RETURN_ON_FALSE(dmx_num >= 0 && dmx_num < DMX_NUM_MAX, ESP_ERR_INVALID_ARG, TAG, "dmx_num error");
    ESP_RETURN_ON_FALSE(dmx_config != NULL, ESP_ERR_INVALID_ARG, TAG, "dmx_config is null");
    ESP_RETURN_ON_FALSE(dmx_config->idle_num <= 0x3ff, ESP_ERR_INVALID_ARG, TAG, "idle_num error");

    // check that the configuration is within DMX specification
    if (!DMX_BAUD_RATE_IS_VALID(dmx_config->baud_rate)) {
        ESP_LOGE(TAG, "baud_rate must be between %i and %i", DMX_MIN_BAUD_RATE, DMX_MAX_BAUD_RATE);
        return ESP_ERR_INVALID_ARG;
    }
    const int brk_us = get_brk_us(dmx_config->baud_rate, dmx_config->break_num);
    if (brk_us < DMX_TX_MIN_SPACE_FOR_BRK_US) {
        ESP_LOGE(TAG, "break must be at least %ius (was set to %ius)", DMX_TX_MIN_SPACE_FOR_BRK_US, brk_us);
        return ESP_ERR_INVALID_ARG;
    }
    const int mab_us = get_mab_us(dmx_config->baud_rate, dmx_config->idle_num);
    if (!DMX_TX_MAB_DURATION_IS_VALID(mab_us)) {
        ESP_LOGE(TAG,
                 "mark-after-break must be between %ius and %ius (was set to "
                 "%ius)",
...
esp_err_t dmx_get_baud_rate(dmx_port_t dmx_num, uint32_t *baud_rate) {
    ESP_RETURN_ON_FALSE(dmx_num >= 0 && dmx_num < DMX_NUM_MAX, ESP_ERR_INVALID_ARG, TAG, "dmx_num error");
    ESP_RETURN_ON_FALSE(baud_rate != NULL, ESP_ERR_INVALID_ARG, TAG, "baud_rate is null");
...
#endif
    ESP_RETURN_ON_FALSE(dmx_num >= 0 && dmx_num < DMX_NUM_MAX, ESP_ERR_INVALID_ARG, TAG, "dmx_num error");
    ESP_RETURN_ON_FALSE(break_num != NULL, ESP_ERR_INVALID_ARG, TAG, "break_num is null");
...
esp_err_t dmx_set_idle_num(dmx_port_t dmx_num, uint16_t idle_num) {
    ESP_RETURN_ON_FALSE(dmx_num >= 0 && dmx_num < DMX_NUM_MAX, ESP_ERR_INVALID_ARG, TAG, "dmx_num error");
    ESP_RETURN_ON_FALSE(idle_num <= 0x3ff, ESP_ERR_INVALID_ARG, TAG, "idle_num error");
...
#endif
    ESP_RETURN_ON_FALSE(dmx_num >= 0 && dmx_num < DMX_NUM_MAX, ESP_ERR_INVALID_ARG, TAG, "dmx_num error");
    ESP_RETURN_ON_FALSE(idle_num != NULL, ESP_ERR_INVALID_ARG, TAG, "idle_num is null");
...
esp_err_t dmx_intr_config(dmx_port_t dmx_num, const dmx_intr_config_t *intr_conf) {
    ESP_RETURN_ON_FALSE(dmx_num >= 0 && dmx_num < DMX_NUM_MAX, ESP_ERR_INVALID_ARG, TAG, "dmx_num error");
    ESP_RETURN_ON_FALSE(intr_conf != NULL, ESP_ERR_INVALID_ARG, TAG, "intr_config is null");
...
esp_err_t dmx_set_rx_full_threshold(dmx_port_t dmx_num, int threshold) {
    ESP_RETURN_ON_FALSE(dmx_num >= 0 && dmx_num < DMX_NUM_MAX, ESP_ERR_INVALID_ARG, TAG, "dmx_num error");
    ESP_RETURN_ON_FALSE(threshold > 0 && threshold < DMX_RXFIFO_FULL_THRESHOLD_MAX, ESP_ERR_INVALID_ARG, TAG,
...
esp_err_t dmx_set_tx_empty_threshold(dmx_port_t dmx_num, int threshold) {
    ESP_RETURN_ON_FALSE(dmx_num >= 0 && dmx_num < DMX_NUM_MAX, ESP_ERR_INVALID_ARG, TAG, "dmx_num error");
    ESP_RETURN_ON_FALSE(threshold > 0 && threshold < DMX_TXFIFO_EMPTY_THRESHOLD_MAX, ESP_ERR_INVALID_ARG, TAG,
...
esp_err_t dmx_read_packet(dmx_port_t dmx_num, void *buffer, uint16_t size) {
    ...
    ESP_RETURN_ON_FALSE(p_dmx_obj[dmx_num] != NULL, ESP_ERR_INVALID_STATE, TAG, "driver not installed");
    ESP_RETURN_ON_FALSE(p_dmx_obj[dmx_num]->buf_size >= size, ESP_ERR_INVALID_ARG, TAG, "size error");
...
esp_err_t dmx_read_slot(dmx_port_t dmx_num, uint16_t slot_idx, uint8_t *value) {
    ...
    ESP_RETURN_ON_FALSE(p_dmx_obj[dmx_num] != NULL, ESP_ERR_INVALID_STATE, TAG, "driver not installed");
    ESP_RETURN_ON_FALSE(p_dmx_obj[dmx_num]->buf_size < slot_idx, ESP_ERR_INVALID_ARG, TAG, "slot_idx error");
...
esp_err_t dmx_write_packet(dmx_port_t dmx_num, const void *buffer, uint16_t size) {
    ...
    ESP_RETURN_ON_FALSE(p_dmx_obj[dmx_num] != NULL, ESP_ERR_INVALID_STATE, TAG, "driver not installed");
    ESP_RETURN_ON_FALSE(p_dmx_obj[dmx_num]->buf_size >= size, ESP_ERR_INVALID_ARG, TAG, "size error");
    ESP_RETURN_ON_FALSE(p_dmx_obj[dmx_num]->mode == DMX_MODE_WRITE, ESP_ERR_INVALID_STATE, TAG, "not in write mode");
...
esp_err_t dmx_send_packet(dmx_port_t dmx_num, uint16_t num_slots) {
    ESP_RETURN_ON_FALSE(dmx_num >= 0 && dmx_num < DMX_NUM_MAX, ESP_ERR_INVALID_ARG, TAG, "dmx_num error");
    ESP_RETURN_ON_FALSE(p_dmx_obj[dmx_num] != NULL, ESP_ERR_INVALID_STATE, TAG, "driver not installed");
    ESP_RETURN_ON_FALSE(p_dmx_obj[dmx_num]->buf_size >= num_slots && num_slots > 0, ESP_ERR_INVALID_ARG, TAG,
                        "num_slots error");
    ESP_RETURN_ON_FALSE(p_dmx_obj[dmx_num]->mode == DMX_MODE_WRITE, ESP_ERR_INVALID_STATE, TAG, "not in write mode");

    // only tx when a frame is not being written
    if (xSemaphoreTake(p_dmx_obj[dmx_num]->tx_done_sem, 0) == pdFALSE) return ESP_FAIL;
    ...
    dmx_hal_write_txfifo(&(dmx_context[dmx_num].hal), zeroeth_slot, num_slots, &bytes_written);
...
esp_err_t dmx_wait_send_done(dmx_port_t dmx_num, TickType_t ticks_to_wait) {
    ...
    if (xSemaphoreTake(p_dmx_obj[dmx_num]->tx_done_sem, ticks_to_wait) == pdFALSE) return ESP_ERR_TIMEOUT;
    xSemaphoreGive(p_dmx_obj[dmx_num]->tx_done_sem);
