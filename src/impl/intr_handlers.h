...
#define UART_INTR_BRK_DET \
    (1 << 7)  // Interrupt that triggers when a break is detected (break bit occurs for longer than a frame length).
#define UART_INTR_RXFIFO_TOUT \
    (1 << 8)  // Interrupt that triggers when the RX FIFO times out waiting for a new frame (mark bit occurs longer than
              // the RX timeout duration).
#define UART_INTR_TX_BRK_DONE (1 << 12)  // Interrupt that triggers when the TX break is finished transmitting.
#define UART_INTR_TX_BRK_IDLE \
    (1 << 13)  // Interrupt that triggers when done TX'ing data, but before the break is finished transmitting.
#define UART_INTR_TX_DONE \
    (1 << 14)  // Interrupt that triggers when finished transmitting data, usually used to indicate a break is ready to
               // be transmitted.
...
#define DMX_INTR_RX_FRAMING_ERR                                                \
    (UART_INTR_PARITY_ERR | UART_INTR_RS485_PARITY_ERR | UART_INTR_FRAME_ERR | \
     UART_INTR_RS485_FRM_ERR)  // Interrupt mask that represents a byte framing error.
#define DMX_INTR_RX_ERR \
    (UART_INTR_RXFIFO_OVF | DMX_INTR_RX_FRAMING_ERR)  // Interrupt mask that represents an error condition.
#define DMX_INTR_RX_ALL                                                \
    (UART_INTR_RXFIFO_FULL | UART_INTR_RXFIFO_TOUT | DMX_INTR_RX_BRK | \
     DMX_INTR_RX_ERR)  // Interrupt mask that represents all rx conditions.

#define DMX_INTR_TX_ALL                                                                           \
    (UART_INTR_TXFIFO_EMPTY | UART_INTR_TX_BRK_IDLE | UART_INTR_TX_DONE | UART_INTR_TX_BRK_DONE | \
     UART_INTR_RS485_CLASH)  // Interrupt mask that represents all tx conditions.
...
static void IRAM_ATTR dmx_intr_handler(void *arg) {
    ...
    while (true) {
        ...
        if (uart_intr_status & UART_INTR_TXFIFO_EMPTY) {
            ...
            dmx_hal_write_txfifo(&(dmx_context[dmx_num].hal), next_slot, num_slots_to_read, &bytes_written);
        ...
        // DMX Receive ####################################################
            ...
            if (rxfifo_len) {
                if (p_dmx->slot_idx < p_dmx->buf_size) {
                    ...
                    dmx_hal_read_rxfifo(&(dmx_context[dmx_num].hal), next_slot, num_slots_to_read);
                    p_dmx->slot_idx += num_slots_to_read;
                    if (uart_intr_status & DMX_INTR_RX_BRK) --p_dmx->slot_idx;  // break is not a slot
                } else {
...
static void IRAM_ATTR dmx_timing_intr_handler(void *arg) {
    ...
    if (dmx_hal_get_rx_level(&(dmx_context[p_dmx->dmx_num].hal))) {
        ...
        if (p_dmx->rx_mab_len == -1 && p_dmx->rx_brk_len != -1) p_dmx->rx_mab_len = now - p_dmx->rx_last_pos_edge_ts;
