...
#define DMX_RX_PACKET_TOUT_TICK \
    ((TickType_t)DMX_RX_PACKET_TOUT_MS / portTICK_PERIOD_MS)  // DMX client packet timeout in FreeRTOS ticks.
...
#define DMX_TX_PACKET_TOUT_TICK \
    ((TickType_t)DMX_TX_PACKET_TOUT_MS / portTICK_PERIOD_MS)  // DMX host packet timeout in FreeRTOS ticks.
 ...
 */
#define DMX_BAUD_RATE_IS_VALID(baud) (baud >= DMX_MIN_BAUD_RATE && baud <= DMX_MAX_BAUD_RATE)
